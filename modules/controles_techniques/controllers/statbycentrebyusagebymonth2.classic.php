        ...
        if($OK == true)
        {
            if(empty($annee) AND empty($centre) AND empty($trimestre)){
                ...
                foreach($_usage as $_usage){
                    ...
                    $_result[$_i]['ssitintotal'] = $myclass->getCompteVisiteByUsageByCentre($code, $usage, $annee, $periode, 1, 1000, 1000, 0, 1);  //total                    // $_result[$_i]['adpartprem'] = $myclass->getCompteVisiteByUsageByCentre($code, $usage, $annee, $periode, 2, 2, 1000, 0);
