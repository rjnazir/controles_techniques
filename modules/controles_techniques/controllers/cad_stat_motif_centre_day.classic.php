...
class cad_stat_motif_centre_dayCtrl extends jController {
    /**
    *
    */
    function index() {
        $rep = $this->getResponse('html');
        $rep->bodyTpl = "controles_techniques~cad_stat_motif_centre_day";
        ...
        if($OK == true)
        {
            if(empty($annee) AND empty($centre) AND empty($trimestre)){
                ...
                $rep->body->assignZone(
                    'res_cad_stat_day',
                    'controles_techniques~res_cad_stat_motif_centre_day',
                    array(
