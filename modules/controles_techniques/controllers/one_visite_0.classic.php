...
class one_visiteCtrl extends jController {
    ...
    */
    function index() {
        ...
        $IMM0 = '';
        if($test == false){
            $IMM0 = substr($IMM, 0, 4).' '.substr($IMM, 4, $long-4);
        }else{
            $IMM0 = substr($IMM, 0, $test).substr($IMM, $test+1, $long-$test);
        }
        $db = jDb::getDbWidget("controles_techniques");
        $db0 = jDb::getConnection("controles_techniques");
        $sql= " SELECT
                    ct_visite.id, ct_visite.vst_num_pv, ct_visite.vst_date_expiration, ct_visite.vst_is_apte, ct_visite.vst_is_contre_visite, ct_visite.vst_created,
                    ct_carte_grise.cg_immatriculation, ct_carte_grise.cg_nom, ct_carte_grise.cg_prenom, ct_carte_grise.cg_profession, ct_carte_grise.cg_nom_cooperative,
                    ct_carte_grise.cg_adresse, ct_carte_grise.cg_commune, ct_carte_grise.cg_puissance_admin, ct_carte_grise.cg_nbr_assis, ct_carte_grise.cg_nbr_debout,
                    ct_carte_grise.cg_num_carte_violette, ct_carte_grise.cg_date_carte_violette, ct_carte_grise.cg_patente, ct_carte_grise.cg_ani,
                    ct_carte_grise.cg_mise_en_service, ct_carte_grise.cg_num_vignette, ct_carte_grise.cg_date_vignette,
                    ct_centre.ctr_nom,
                ...
                FROM   ct_visite LEFT JOIN
                    ...
                    ct_utilisation ON ct_utilisation.id = ct_visite.ct_utilisation_id
                WHERE
                    (ct_carte_grise.cg_immatriculation = '".$IMM."' OR ct_carte_grise.cg_immatriculation = '".$IMM0."')
                    AND `ct_visite`.`vst_created` =  (SELECT MAX(`ct_visite`.`vst_created`) FROM ct_visite LEFT JOIN
                    `ct_carte_grise` ON `ct_carte_grise`.`id` = `ct_visite`.`ct_carte_grise_id`
                    WHERE (`ct_carte_grise`.`cg_immatriculation` = '".$IMM."' OR `ct_carte_grise`.`cg_immatriculation` = '".$IMM0."'))";
